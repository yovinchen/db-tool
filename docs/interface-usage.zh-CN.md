...
- `--limit` 必须大于零。`--max-bytes` 默认 8 MiB、必须在 `1..=16 MiB`；SQL/CQL、
  Document 与消息批次使用它限制一个完整调用方响应。已经声明 bounded operation 的用户查询和顶层名称目录，在可分页
...
`export kv` 当前使用 `--limit + 1` 探测额外 key；`export doc` 使用与 `doc find` 相同的
`ReadBudget(--limit,--max-bytes)`。两者都只把前 `--limit` 项写入 artifact。当前格式分别为
