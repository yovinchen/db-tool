...
- `--limit` 必须大于零。已经声明 bounded operation 的用户查询和顶层名称目录，在可分页
...
let needed = CapabilityOperation::SqlQueryBounded;
...
let rows = sql.query_bounded("select 1 as id", &[], 100).await?;
...
SQL 原子导入、KV expiry、Document one/many 与 drop、所有 bounded catalog、Messaging
...
`--limit` 项写入 artifact。当前格式分别为 `kv-pairs` v3、`documents` v3，并固定包含：
...
`kv.get_with_expiry`，由 adapter 在一个后端操作内读取 value、Redis `TIME` 与 `PTTL`；
