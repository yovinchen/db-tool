...
```bash
./scripts/validate-release-version.sh v1.0.0
```
...
所有已经存在 exact sibling 的无界、仅按条数或无输入信封方法都只作为 `1.x`
嵌入式兼容面保留，并带有 Rust `#[deprecated]` 迁移提示；最早只能在 breaking
release `2.0.0` 删除。旧 `CapabilityOperation` 名仍保留序列化兼容，但它们只说明旧
实现存在，不能授权 exact 调用，新调用方也不得在 exact operation 缺失时回退：

| Trait | `1.x` legacy 方法 | 新调用方必须协商并调用 |
| --- | --- | --- |
...
`1.x` legacy-to-legacy 默认桥接；它们不是 exact 默认实现。其余保留调用
只出现在带局部 `#[allow(deprecated)]` 的 `1.x compatibility` 测试中。
