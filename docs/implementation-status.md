...
| IBM Db2 | `db2://` | Db2 ODBC | `ping`, `sql query`, `sql exec`, `sql tables`, `sql schema`, `db2 schemas`, `db2 tables`, `db2 schema`, `db2 sequences`, `db2 routines`, `db2 tablespaces`, `db2 foreign-keys`, `db2 ddl` | Service-free exact operation/budget tests pass. The pinned V12.1 product becomes healthy on GitHub x86_64 and its `libdb2o` passes architecture/dependency/registration checks, but copying server libraries onto a generic hosted runner fails IBM environment allocation (`IM004`). Full LIVE_PASS requires the labeled self-hosted runner with an installed 64-bit IBM client. |
...
| IBM Db2 end-to-end live Docker run | Requires a supported host-installed IBM 64-bit ODBC runtime | The adapter, pinned Docker product profile, integration script, and `db2-client` self-hosted workflow lane exist. Generic hosted experiments proved that extracting/registering container `libdb2o` is insufficient: environment allocation fails with `IM004` before mutations. Provision an x86_64 runner with the IBM client installed, `db2profile` sourced, and `IBM DB2 ODBC DRIVER` registered; service-free adapter tests pass everywhere. |
