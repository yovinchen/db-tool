...
| Production TiDB HA | Partial | Local secure HA topology, SQL-node failover drill, PD single-node outage drill, PD leader outage drill, certificate regeneration cold-restart drill, and TiProxy new-connection routing drill are available, but TiKV failure, online certificate rotation, backup/restore, and upgrade drills are not covered. | Add product-specific production drills only when production-readiness is in scope. |
