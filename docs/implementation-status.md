...
| Redis | `redis://` | Redis | KV get/set/delete/scan/raw, TTL, Streams, Pub/Sub | Redis 7.4.9 Docker exact N/N+1 truncation, 25-key multi-page SCAN, strict non-UTF-8 error, transfer and messaging tests |
...
| Kafka native | `kafka://` with `full-native` | Kafka | librdkafka-backed field-fidelity/cursor produce-consume, topics/detail/delete, and real committed-offset consumer lag | Native field fidelity, group lag and deletion tests passed against Redpanda |
...
| KV | `kv get`, `kv set`, `kv scan`, `kv del`, `kv raw` | `set`, `del`, and mutating raw commands require `--allow-write`; scan performs an adapter-level N+1 probe, deduplicates Redis pages, propagates page/decode errors, and marks truncation only when an extra key exists |
...
| Messaging | `mq produce`, `mq consume`, `mq topics`, `mq detail`, `mq lag`, `mq delete` | produce requires `--allow-write`; consume requires positive max/timeout and exposes native inclusive cursor plus lossless metadata where available; AMQP consume requires write permission because it ACKs; persistent-resource delete requires `--allow-write` and a token bound to kind/name/options; reaching max is a budget signal, not proof of another message |
