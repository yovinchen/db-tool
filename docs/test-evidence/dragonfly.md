...
Command: `DBTOOL_IT_COMPAT_EXTRA=1 ./scripts/integration-compat-test.sh`; `DBTOOL_RUN_INTEGRATION=1 DBTOOL_IT_REDIS_DSN=dragonfly://127.0.0.1:26381/0 cargo test -p dbtool-cli --test transfer_artifacts redis_artifact_v3_preserves_lifetimes_skips_expired_and_binds_replacement -- --exact --nocapture`
...
scan, exact deletion count, and complete post-delete absence all passed. The v3
refresh also proved integer `TIME` compatibility, per-entry atomic lifetime
restore, and no expired-key revival.
