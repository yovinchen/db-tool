...
Boundaries: the typed TOML model preserves modeled defaults, connections, readonly flags and limits, but does not retain comments or source formatting. The CLI reports this as `comments_preserved=false`. Windows replacement uses `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)`; it was not runtime-executed in this original macOS slice, but the later Windows x64 runtime and ARM64 compile/link gates passed in [run 29599157939](https://github.com/yovinchen/db-tool/actions/runs/29599157939), with details in [`transfer-artifact-atomic-publication.md`](transfer-artifact-atomic-publication.md).
