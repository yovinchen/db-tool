...
| SQL Server | 同 SQL operations | checked `TOP (N+1)`；TDS row 转换前受固定边界 | service-free 8/8 精确边界 PASS；后续 x86_64 产品目录生命周期 LIVE_PASS，见 `sqlserver.md` |
...
该 exact operation；这不是用空列表伪装成功。Redshift、SQL Server、Db2、外部 Kafka
vendors 与 Elasticsearch 原生 HTTPS 的额外产品运行仍由 IF-T52 跟踪。真实 ScyllaDB
后续已完成独立产品 CRUD/目录/读取预算门禁，见 [`scylladb.md`](scylladb.md)；本节原
IF-T74 的逐字节目录矩阵仍只代表当时明确列出的产品运行。
