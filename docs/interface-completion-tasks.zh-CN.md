...
| IF-T46 Document 集合生命周期与查询选项 | P1 | 待实施 | MongoDB CLI 未暴露 `skip/sort/projection`，公共 API 不能 drop collection。 | `doc find` 暴露完整 `FindOptions`；增加集合删除能力和目标绑定确认；MongoDB 精确查询与清理实测。 |
...
| IF-T54 TUI 安全与终端恢复 | P0 | 待实施 | TUI 依赖字符串前缀识别写操作，SQL fallback 可绕过共享 `SafetyGuard`；异常退出可能残留 raw mode/alternate screen。 | 所有 SQL 输入先走共享分类和 readonly/确认检查；新增绕过回归测试；使用 RAII terminal guard 保证正常、错误和 panic unwind 均恢复终端。 |
