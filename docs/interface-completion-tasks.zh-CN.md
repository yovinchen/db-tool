...
| IF-T50 设计与状态文档同步 | P1 | 完成 | `dbtool-design.md`、README、implementation status 的 feature、命令和更新时间曾落后于实际代码。 | 设计文档现只列已注册 feature/scheme、真实 CLI/TUI/embedded 能力和发布边界；已解决问题归档；README、Skill、中文使用范式、实现状态与证据索引同步到 2026-07-18。 |
| IF-T51 编译与打包闭环 | P0 | 完成（单一 macOS ARM64 发布目标） | 接口收口后已完整执行 fmt/workspace check/strict Clippy/全仓测试、deprecated production gate、minimal/default/portable/full/full-native feature 矩阵、portable release 与 Docker 冷构建。正式发布范围按产品决策收缩为 `aarch64-apple-darwin`。 | 本机一键脚本生成 binary、completions、manpage、archive 与 SHA-256，解包运行和 SQLite 核心流程 PASS；GitHub Release 使用真实 ARM64 `macos-latest` runner 构建/执行并只上传一个 `.tar.gz`。可选 npm 路径另有 6 个平台包 + 1 个入口包的闭合矩阵、许可证、离线安装和 dry-run 门禁，但未冒充正式 npm 发布。证据：[`release-packaging.md`](test-evidence/release-packaging.md)、[`npm-publication.md`](test-evidence/npm-publication.md)。 |
| IF-T52 外部产品验证 | P2 | 外部阻塞 | Db2 需要正式安装并配置 IBM 64 位客户端的 `db2-client` self-hosted x86_64 runner；Redshift、AutoMQ/WarpStream/Confluent 与 Elasticsearch 原生 HTTPS 仍依赖额外端点、凭据或产品安全环境。 | 保留独立 runner 和解除条件；获得相应环境后逐产品生成 LIVE_PASS 证据；没有真实运行不得改成 COMPLETE。Db2 的 pinned V12.1 产品已健康、`libdb2o` 架构/依赖/注册已通过，但复制服务器库在 `SQLAllocHandle(HENV)` 返回 `IM004`，因此没有伪报 CRUD PASS。SQL Server 已在官方 x86_64 产品容器完成生命周期，ScyllaDB 已由 IF-T80 完成产品原生验证。 |
...
| IF-T67 二级元数据有界化 | P0 | 完成 | `MetadataBudget(max_items,max_bytes)` 已覆盖 SQL/CQL/Db2 完整表结构与 DDL、AMQP/Kafka/Redis/NATS detail 和 lag；columns、index membership、config、watermark 与 lag 候选均在保留前计费。CLI/TUI 只协商 exact bounded operation，`--limit/--max-bytes` 在 DSN 前校验；超限稳定返回 `METADATA_BUDGET_EXCEEDED`，不返回部分 schema/detail/DDL。 | core、SQLx、SQL Server、Cassandra、Db2 和四类消息 adapter 的 N/N+1、字节 N/N-1、协议 frame/分页边界均通过；SQL Server 产品目录生命周期已在 x86_64 LIVE_PASS，精确 N/N-1/byte 边界仍由 adapter 证据证明；Db2 live 归 IF-T52。证据：[`nested-metadata-budgets.md`](test-evidence/nested-metadata-budgets.md)。 |
| IF-T68 Transfer artifact 跨平台原子替换 | P1 | 完成 | transfer 与连接配置已复用同一原子发布 primitive；Windows 使用 replace-existing + write-through，Unix rename 后同步父目录；替换前失败删除临时文件且旧目标保持完整。PR 门禁显式运行 core 原子测试、CLI transfer 替换测试，并对 Windows x64/arm64 portable release 完整编译链接。 | 本机 core 3/3、CLI transfer 1/1、portable check 和 YAML 解析通过；GitHub Actions Windows x64 编译、链接及 SQLite/拒绝写 runtime smoke PASS，Windows ARM64 编译链接 PASS。证据：[`transfer-artifact-atomic-publication.md`](test-evidence/transfer-artifact-atomic-publication.md)。 |
| IF-T69 SQL/CQL 结果递归字节预算 | P0 | 完成 | 新增 `ReadBudget`、`sql.query_budgeted/cql.query_budgeted` 与稳定 `READ_BUDGET_EXCEEDED`；列元数据、完整 row、递归 Value、最终 ResultSet 和 N+1 probe 共用一个字节信封。SQLx、Cassandra、Tiberius、Db2 均在协议读取边界限制观察/预取；CLI/TUI/export 只调用 exact operation，并暴露全局 `--max-bytes`。 | core/五类 adapter 单测与 Clippy 通过；SQLite 字节边界、Docker PostgreSQL 16/MySQL 8/Cassandra 5 的 N/N+1 与小字节预算由 CLI 实测；SQL Server 产品生命周期已通过且精确字节边界由 adapter 测试证明；Db2 live 归 IF-T52。证据：[`read-budget-envelopes.md`](test-evidence/read-budget-envelopes.md)。 |
...
| IF-T81 外部产品 runner 防假绿 | P0 | 完成 | Redshift 与 AutoMQ/WarpStream/Confluent 脚本在无 DSN 时 exit 0，db-suite 和手动 CI 可能把 SKIP 误显示为 PASS。 | 本地直接运行无 DSN 时明确输出 SKIP；db-suite 将其归类到 skipped 列表且不打印 passed；CI 手动请求外部作业时固定 `DBTOOL_IT_REQUIRE_EXTERNAL=1`，缺 secret 必须 exit 2；service-free 回归覆盖 skip、strict fail、fake cargo 真执行和 sentinel 凭据不回显。 |

## 本轮完整性与稳定性补充任务

这些任务不扩充数据库产品清单，只加固现有接口、证据和跨端 runner。

| Task | 优先级 | 状态 | 结果 |
| --- | --- | --- | --- |
| AUD-T01 27 项完整性账本强校验 | P0 | 完成 | manifest 与人工 ledger 双向 ID/状态一致；缺失、重复、额外、伪造状态、无 cleanup/result 证据和不可达提交均 fail closed；fixture 已接入 `verify.sh`。 |
| AUD-T02 CLI 参数错误机器输出 | P1 | 完成 | `--json-errors` 将 Clap 参数失败稳定输出为 `CLI_ARGUMENT_ERROR` JSON 和退出码 2，同时保持默认文本及 help/version 兼容。 |
| AUD-T03 产品 runner feature 隔离 | P0 | 完成 | SQL Server、Cassandra/Scylla、Db2、纯/原生消息只编译自身依赖；无关 Db2 ODBC 不再阻塞其它产品测试，feature matrix 防止回归。 |
| AUD-T04 发布依赖安全整改 | P0 | 上游阻塞 | 5 条 open Dependabot 告警已逐依赖路径审计，但 Tiberius 无修复版、async-nats 需要 MSRV/API 大升级、稳定 Ratatui 仍固定旧 lru；解除条件见 [`dependency-security-audit.md`](test-evidence/dependency-security-audit.md)。在真实升级完成前不得宣称零已知安全告警。 |
