...
| IF-T48 CLI 参数面完整性 | P1 | 实施中（Kafka、Redis 完成；NATS/AMQP 待完成） | Search 聚合及完整响应、TS start/end、MQ key/header/partition/timestamp/offset、lossless cursor 与非 Kafka metadata 已补齐；group/durable/ack 已冻结为 typed identity/ack 与独立 operation。native Kafka 已完成 group subscription/commit；Redis 已完成显式 group+member、PEL 优先重放、整批无损转换后 XACK、运行时 lag 协商与公共 CLI 实测。 | Search、TS、native Kafka 与 Redis Streams 已完成；Kafka/Redis Streams/NATS JetStream 的无状态 cursor 继续兼容；完成 NATS queue/durable ACK 和 AMQP 批量成功后 ACK 后整体标记完成。 |
