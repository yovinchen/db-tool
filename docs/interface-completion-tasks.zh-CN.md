...
| IF-T48 CLI 参数面完整性 | P1 | 实施中（typed group/durable/ack 合同完成，逐协议实现中） | Search 聚合及完整响应、TS start/end、MQ key/header/partition/timestamp/offset、lossless cursor 与非 Kafka metadata 已补齐；group/durable/ack 已冻结为 typed identity/ack 与独立 operation，但各 broker 的 commit/PEL/durable/ACK 仍需分别兑现。 | Search 与 TS 参数面已完成；Kafka/Redis Streams/NATS JetStream 的无状态 cursor 继续兼容；完成 native Kafka group commit、Redis XREADGROUP/XACK、NATS queue/durable ACK 和 AMQP 批量成功后 ACK 后整体标记完成。 |
...
| IF-T64 KV 过期时间无损迁移 | P0 | 实施中 | KV v2 artifact 只保存 value，导入时统一 `--ttl` 会把 persistent 与 expiring key 混为一谈，并可能延长或复活源 key。 | 以可协商的原子 value+absolute-expiry 读写合同生成 v3 artifact；过期 entry 不落盘、不复活；v2 在联网前明确拒绝；Redis/Valkey/KeyDB/Dragonfly 做真实生命周期验证。 |
