...
| IF-T48 CLI 参数面完整性 | P1 | 实施中（TS/Search/消息字段/cursor 已完成） | Search 聚合及完整响应、TS start/end、MQ key/header/partition/timestamp/offset、lossless cursor 与非 Kafka metadata 已补齐；设计声明的 group/durable/ack 消费语义仍在收口。 | Search 与 TS 参数面已完成；Kafka/Redis Streams/NATS JetStream 的输出 cursor 可作为同一消息的 inclusive 输入，AMQP metadata 明确为已 ACK 诊断值；完成 group/durable/ack 后整体标记完成。 |
