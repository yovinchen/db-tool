...
| IF-T76 Search 完整读取信封 | P0 | 待实施 | Search hit 数量虽由请求 `size` 限制，但完整 `_source`、aggregations、hits metadata、unknown `extra` 与 get document 只有固定 16 MiB HTTP 上限，没有 caller `--max-bytes` 或 exact budgeted operation。 | 增加 `search_budgeted/get_doc_budgeted` 与 exact operation；HTTP body 在 JSON 解析前受 caller/16 MiB 较小值限制，完整 portable 响应再次精确计费；OpenSearch 与 Elasticsearch 分别 live 验证并删除测试索引。 |
| IF-T77 MessageProducer 输入预算与结果不确定性 | P0 | 待实施 | `produce(Vec<Message>)` 没有消息数、单消息或整批预算；Kafka、AMQP、NATS、Redis 的逐条/逐分区发送可能在后项失败前已写入前项，却返回可被误重试的普通错误。 | 增加 `ProduceBudget`、`produce_budgeted` 与 exact operation；所有字段/数量/字节/协议限制在任何建资源或发送前预检；首次可能写入后的失败统一为非重试 `OUTCOME_INDETERMINATE`；四协议 Docker 证明超限零写入、成功可读回并清理。 |
| IF-T78 通用 mutation/request 输入信封 | P1 | 待实施 | SQL/CQL execute、KV mutation、Document mutation/batch、TS write 与 Search mutation 等 public trait 仍可由 embedded 调用方传入无界参数/文档/点/键；部分 adapter 还会构造多份完整编码或产生批次部分成功。 | 新增通用 `InputBudget/InputLimiter`，以兼容 sibling 方法和 exact operation 按协议族迁移；所有结构、数量、递归字节与 adapter 固定协议上限在连接/首个写入前验证；原子与非原子结果语义分别记录。 |
| IF-T79 本地连接配置有界化 | P2 | 待实施 | `ConnectionConfig::load` 全量 `read_to_string`，环境连接全量收集，`conn list` 绕过数据命令的 `--limit/--max-bytes`，超大配置/DSN 可无界占用内存与输出。 | 固定配置文件/单 DSN/条目/序列化硬上限；load/save 在完整分配或替换前失败；`conn list` 使用 items+bytes 信封并保留脱敏、原子替换及旧配置兼容。 |
