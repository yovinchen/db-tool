...
| IF-T67 二级元数据有界化 | P0 | 待实施 | 顶层目录已受控，但 `TableSchema.columns/indexes`、Cassandra/Db2 DDL 所依赖的完整表结构、Kafka topic partition watermarks 与 consumer lag 明细等单资源内部集合还没有统一调用方预算。 | 为可增长的二级集合增加显式预算/截断模型；adapter 在协议边界停止或使用硬响应预算；Db2 DDL 等必须完整才能生成的接口设置明确大小上限并失败关闭；CLI/embedded 不得静默截断语义完整对象。 |
| IF-T68 Transfer artifact 跨平台原子替换 | P1 | 待实施 | artifact 已使用同目录临时文件、sync 与 rename；Unix 可替换已有目标，但 Windows `fs::rename` 不具备 replace-existing 语义，与配置 CRUD 的跨平台合同不一致。 | 抽取或复用受测的原子发布 primitive；Windows 使用 replace-existing + write-through，Unix rename 后同步父目录；失败删除临时文件且旧目标保持完整；增加平台无关覆盖与 Windows 编译门禁。 |
