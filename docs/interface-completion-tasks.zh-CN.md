...
| IF-T52 外部产品验证 | P2 | 外部阻塞 | Db2 ODBC、Redshift、AutoMQ/WarpStream/Confluent 与 Elasticsearch 原生 HTTPS 仍依赖额外运行时、端点或凭据。 | 保留独立 runner 和解除条件；获得相应环境后逐产品生成 LIVE_PASS 证据；没有真实运行不得改成 COMPLETE。SQL Server 已在官方 x86_64 产品容器完成生命周期，ScyllaDB 已由 IF-T80 完成产品原生验证；本地 Apple Silicon 不能运行 SQL Server Linux 镜像只是架构边界，不再计为未完成产品任务。 |
