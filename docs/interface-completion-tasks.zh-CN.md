...
| IF-T48 CLI 参数面完整性 | P1 | 实施中（TS/Search 已完成） | Search 聚合及完整响应、TS start/end 已补齐；MQ key/header/partition/offset/cursor/group 仍在收口。 | Search 已返回聚合与后端元数据；TS 已支持互斥的最近窗口和成对 epoch-ms 范围并完成 mock/Prometheus 实测；待完成 MQ 参数与协议边界后整体标记完成。 |
