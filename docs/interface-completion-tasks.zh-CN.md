...
| IF-T67 二级元数据有界化 | P0 | 实施中 | 顶层目录已受控，但 `TableSchema.columns/indexes`、Cassandra/Db2 DDL 所依赖的完整表结构、Kafka topic partition watermarks 与 consumer lag 明细等单资源内部集合还没有统一调用方预算。 | 使用完整对象 `MetadataBudget(max_items,max_bytes)`；columns、index identity、index membership、topic config/watermark 和 lag 候选统一计费；adapter 在协议边界执行 remaining+1 或硬响应预算；超限返回 `METADATA_BUDGET_EXCEEDED`，绝不返回部分 schema/detail/DDL。 |
| IF-T68 Transfer artifact 跨平台原子替换 | P1 | 实施中 | artifact 已使用同目录临时文件、sync 与 rename；Unix 可替换已有目标，但 Windows `fs::rename` 不具备 replace-existing 语义，与配置 CRUD 的跨平台合同不一致。 | 抽取或复用受测的原子发布 primitive；Windows 使用 replace-existing + write-through，Unix rename 后同步父目录；失败删除临时文件且旧目标保持完整；增加平台无关覆盖与 Windows 编译门禁。 |
| IF-T69 SQL/CQL 结果递归字节预算 | P0 | 待实施 | `query_bounded/query_cql_bounded` 只限制行数；单行可以包含大量列、超大 Text/Bytes/JSON，数组和 Map 还能递归增长，跨行总结果也没有调用方字节预算。 | 新增稳定的结果读取预算与精确 operation；列元数据、每个完整行及递归 Value 在保留前计费；adapter 能配置时冻结协议 frame 上限；CLI/TUI/export 只调用新接口，首行或总量超限 fail closed。 |
| IF-T70 KV/Redis 读取信封 | P0 | 待实施 | `get/get_with_expiry`、普通 SCAN 与 typed RAW 会先让 Redis 客户端解码完整 bulk/array，之后才检查数量或返回大小；export 还会复制完整 value。 | 增加 `get_bounded/get_with_expiry_bounded/scan_bounded/raw_command_bounded` 及 exact operation；用服务端 Lua/协议上限在返回前检查 value/page items/page bytes；CLI 与 transfer 不保留第二份无界副本。 |
| IF-T71 Document 累计字节预算 | P0 | 待实施 | MongoDB 的 16 MiB 单文档和 48 MiB wire message 只限制单元，`find/aggregate` 仍可跨 batch 累积大量文档；CLI 的 N+1 仅按文档数。 | 增加文档总 items+bytes 预算；按原始/编码 BSON 大小逐文档计费，超限不返回部分成功；find、aggregate、CLI 和 export 使用同一 caller budget。 |
| IF-T72 Messaging 消费字节与 ACK 语义 | P0 | 待实施 | consume 只限制消息数和时间；payload、key、headers、cursor/meta 及整个 batch 没有字节上限，且超限必须发生在 ACK/XACK/commit 前。 | ConsumeOptions 增加单消息与批次硬预算；四类 adapter 在确认前完成转换和计费，超限保持可重投/未提交状态；Kafka 接收 frame 上限不能被 DSN 放大。 |
| IF-T73 Time-series 结构化预算 | P1 | 待实施 | HTTP body 已有 16 MiB 硬上限，但 series、label、column、sample 在完整 JSON 解析后才由 CLI 按 sample 数截断，caller 无法分别限制结构规模。 | 增加 `query_range_bounded(max_series,max_samples,max_bytes)`；adapter 内部对完整 series 与 sample 计费并保留 16 MiB transport ceiling；CLI/TUI 按 exact operation 调用。 |
| IF-T74 目录标量字节预算 | P1 | 待实施 | 通用 `ListLimiter` 只按条目计数；SQLite 等后端的单个目录名称没有统一 caller byte budget，少量超长标量仍可占用大响应。 | 引入 items+bytes 的列表预算或复用统一读取预算；所有可增长字符串目录在 retention 前计费，现有 N/N+1 completeness 语义保持不变。 |
| IF-T75 旧无界 API 生命周期 | P2 | 待实施 | 为嵌入式兼容保留的 legacy `query/list/find/aggregate/topic_detail` 等方法仍可被新调用方直接使用；CLI 已逐步迁移但 API 没有清晰弃用路径。 | 文档标注兼容用途并添加 Rust deprecation/迁移说明；所有一方调用路径只使用 exact bounded operation；保留明确版本窗口，不在小版本中破坏外部实现。 |
