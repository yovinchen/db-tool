...
| IF-T51 编译与打包闭环 | P0 | 实施中（本机最终门禁 PASS，等待六平台 CI） | 接口收口后已完整执行 fmt/workspace check/strict Clippy/全仓测试、deprecated production gate、minimal/default/portable/full/full-native feature 矩阵、portable release 与 Docker 冷构建。 | macOS arm64 binary、completions、manpage、archive、npm tarball、wheel 已生成；archive 烟测、npm offline 隔离安装、Python `--no-index` venv 安装和 Docker 非 root SQLite 流程均 PASS。关闭本 Task 仍需 GitHub Actions 六目标真实 binary/install gate，本机不伪造其他五平台证据。证据：[`release-packaging.md`](test-evidence/release-packaging.md)。 |
...
| IF-T75 旧无界 API 生命周期 | P2 | 完成 | Core 9 个 trait 的 64 个无界/仅 item-bound 方法已统一标记 Rust `#[deprecated]`，每个 note 指向 exact replacement；兼容面在整个 `0.1.x` 保留，最早只能在 `0.2.0` 删除。 | CLI/TUI/transfer/embedded/全部 adapter production target 以 `RUSTFLAGS='-D deprecated' cargo check --workspace --lib --bins` 通过；exact 默认实现不回退 legacy，仅保留两个明确的 `0.1.x` legacy-to-legacy Document 默认桥接和局部兼容测试。证据：[`legacy-api-lifecycle.md`](test-evidence/legacy-api-lifecycle.md)。 |
