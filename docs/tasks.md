...
| IF-T43–IF-T79 Interface completion campaign | P0 | In progress | IF-T43–50, IF-T53–67, and IF-T69–79 are complete (34 tasks). IF-T51's final macOS arm64 compile/test/package/Docker gate is PASS and only the six-platform CI matrix remains. IF-T52 remains externally blocked. IF-T68 is implemented but still awaits the current SHA's Windows x64 runtime and arm64 compile/link evidence. External runtime prerequisites are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
