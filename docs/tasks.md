...
| IF-T43–IF-T68 Interface completion campaign | P0 | In progress | IF-T43 through IF-T66 are complete: declared method gaps, feature/release mismatches, TUI safety, CLI usage, exact capability negotiation, stateful messaging, transfer fidelity, and top-level bounded catalogs are implemented and independently evidenced. IF-T67 closes nested metadata collections and complete-object size ceilings; IF-T68 aligns transfer artifact atomic replacement across Unix and Windows; IF-T51 then performs the final all-feature compile/test/package pass. External runtime prerequisites remain separately blocked and are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
