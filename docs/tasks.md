...
| IF-T43–IF-T75 Interface completion campaign | P0 | In progress | IF-T43 through IF-T66 are complete. A second read-envelope audit added IF-T67 through IF-T75 so nested metadata, cross-platform artifact publication, recursive SQL/CQL values, Redis/KV reads, multi-document accumulation, message batch bytes/ACK ordering, time-series structure, catalog scalar bytes, and legacy unbounded API lifecycle are visible rather than hidden behind top-level item limits. IF-T51 performs the final all-feature compile/test/package pass only after these interfaces close. External runtime prerequisites remain separately blocked and are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
