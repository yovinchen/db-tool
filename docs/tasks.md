...
| IF-T43–IF-T81 Interface completion campaign | P0 | In progress | IF-T43–51, IF-T53–67, and IF-T69–81 are complete (37 tasks). IF-T51 is complete for the selected single macOS ARM64 release. IF-T52 remains externally blocked. IF-T68 is implemented but still awaits the current SHA's Windows x64 runtime and arm64 compile/link evidence and no longer blocks the macOS-only release. External runtime prerequisites are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
