...
| T16 Production TiDB HA drills | P3 | In progress | Added a secure HA SQL-node failover drill plus a TiProxy drill. The TiProxy drill exposes a TLS proxy entrypoint, verifies a `REQUIRE SSL` user through the proxy, and proves new proxy connections can keep writing/reading while each TiDB SQL node is stopped in turn. PD/TiKV failover, certificate rotation, backup/restore, upgrade, and existing-session migration drills remain deferred. | `./scripts/integration-tidb-ha-drill.sh`; `./scripts/integration-tidb-tiproxy-test.sh`; `run_live_tidb_ha_drill` and `run_live_tidb_tiproxy` workflow inputs; documented pass/fail criteria in `docs/tidb-compat-design.md`. |
