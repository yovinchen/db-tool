...
| IF-T43–IF-T79 Interface completion campaign | P0 | In progress | IF-T43 through IF-T67 plus IF-T69 through IF-T72 are complete. IF-T68 is implemented but still awaits the current SHA's Windows CI evidence. A strict public-trait/CLI audit added IF-T76 Search read envelopes, IF-T77 producer input/outcome safety, IF-T78 general mutation input envelopes, and IF-T79 bounded local connection configuration; they remain open with IF-T73, IF-T74, and IF-T75. IF-T51 performs the final all-feature compile/test/package pass only after these interfaces close. External runtime prerequisites remain separately blocked and are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
