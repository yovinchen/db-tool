...
- [x] Local DB suite runner for phase-based service-free, base, fixture, roundtrip, compatibility, TiDB, and opt-in heavy profile automation.
...
| T16 Production TiDB HA drills | P3 | In progress | Added a secure HA SQL-node failover drill, a PD single-node outage drill, a PD leader outage drill, a TiKV outage boundary drill, a certificate regeneration cold-restart drill, a logical data roundtrip smoke, and a TiProxy drill. The PD drill stops each PD node one at a time and proves both SQL nodes continue TLS writes/reads; the leader drill discovers the current PD leader through the TLS PD API and proves SQL continuity after stopping it; the TiKV boundary drill stops one TiKV and proves dbtool SQL probes either continue or return bounded failures; the certificate drill proves regenerated CA/server/client certificates are loaded after a cold restart; the logical roundtrip smoke exports rows through one TLS SQL node, restores through the other, and reads back from both; the TiProxy drill exposes a TLS proxy entrypoint, verifies a `REQUIRE SSL` user through the proxy, and proves new proxy connections can keep writing/reading while each TiDB SQL node is stopped in turn. Production TiKV failover, online certificate rotation, product-native backup/restore, upgrade, and existing-session migration drills remain deferred. | `./scripts/integration-tidb-ha-drill.sh`; `./scripts/integration-tidb-pd-drill.sh`; `./scripts/integration-tidb-pd-leader-drill.sh`; `./scripts/integration-tidb-tikv-outage-boundary.sh`; `./scripts/integration-tidb-cert-regeneration-test.sh`; `./scripts/integration-tidb-logical-roundtrip-test.sh`; `./scripts/integration-tidb-tiproxy-test.sh`; local script validation while CI budget is frozen; documented pass/fail criteria in `docs/tidb-compat-design.md`. |
