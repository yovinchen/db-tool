...
| T16 Production TiDB HA drills | P3 | In progress | Added a secure HA SQL-node failover drill, a PD single-node outage drill, and a TiProxy drill. The PD drill stops each PD node one at a time and proves both SQL nodes continue TLS writes/reads; the TiProxy drill exposes a TLS proxy entrypoint, verifies a `REQUIRE SSL` user through the proxy, and proves new proxy connections can keep writing/reading while each TiDB SQL node is stopped in turn. TiKV failover, leader-specific PD targeting, certificate rotation, backup/restore, upgrade, and existing-session migration drills remain deferred. | `./scripts/integration-tidb-ha-drill.sh`; `./scripts/integration-tidb-pd-drill.sh`; `./scripts/integration-tidb-tiproxy-test.sh`; local script validation while CI budget is frozen; documented pass/fail criteria in `docs/tidb-compat-design.md`. |
