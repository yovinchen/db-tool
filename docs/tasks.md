...
| IF-T43–IF-T79 Interface completion campaign | P0 | In progress | IF-T43 through IF-T67, IF-T69 through IF-T74, IF-T76, and IF-T79 are complete. IF-T68 is implemented but still awaits the current SHA's Windows CI evidence. IF-T75, IF-T77, and IF-T78 remain open. IF-T51 performs the final all-feature compile/test/package pass only after every in-scope interface closes. External runtime prerequisites remain separately blocked and are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
