...
| IF-T43–IF-T61 Interface completion campaign | P0 | In progress | Close the remaining declared-interface gaps, feature/release mismatches, TUI safety holes, CLI usage and capability negotiation; synchronize the current design and rebuild/package the project. Redis exact SCAN, SQL transactional import, messaging resource lifecycle, RabbitMQ fail-closed management detail, atomic named-connection CRUD, and explicit Document one/many mutation semantics are included in the evidence ledger; external product/runtime prerequisites remain separately blocked. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
