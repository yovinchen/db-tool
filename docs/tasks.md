...
- [x] Exact `ProduceBudget` and `message.produce_budgeted` contract with CLI pre-DSN validation and non-retryable post-dispatch timeout semantics.
- [x] Kafka pure/native, RabbitMQ, NATS, and Redis producer N/N-1, oversize zero-write/no-resource, readback, and scoped cleanup evidence.
- [x] Preserve 0.x legacy empty-batch no-op only after valid-target checking; reject empty exact batches and bound every non-empty legacy batch.
...
| T40 IBM Db2 adapter | P3 | Done (live product externally blocked) | Added `adapter-db2` using ODBC, DSN scheme `db2://`, aliases `ibmdb2://`/`as400://`, full `SqlEngine` surface, a pinned V12.1 Docker product profile, and lifecycle tests. Service-free exact operations and budgets pass. Hosted experiments reached product health and registered dependency-complete x86-64 `libdb2o`, but a server-library transplant failed environment allocation with `IM004`; supported LIVE_PASS now requires the labeled self-hosted runner with an installed IBM client. | `cargo test -p adapter-db2`; `docker compose -f docker-compose.integration.yml --profile db2 config`; `docs/test-evidence/db2-host-runtime-blocker.md`. |
...
| T42 Real backend completeness campaign | P1 | Done | Every locally available registered backend completed its family checklist. The current campaign state is 24 COMPLETE, 1 BLOCKED, 2 EXTERNAL, and 0 PARTIAL; unavailable architecture/runtime/credential boundaries remain explicit rather than being treated as passes. | `docs/db-completeness-tasks.md`; `testdata/db-completeness.manifest`; `./scripts/validate-db-completeness.sh`. |
| T43 Product-native ScyllaDB gate | P1 | Done | Replaced Cassandra-only alias confidence with a dedicated official ScyllaDB 2026.1.8 ARM64 profile and real product test. The shared CQL contract now runs CRUD, typed values, result/catalog bounds, exact input budgets, and cleanup against `scylla://`. | `./scripts/integration-scylla-test.sh`; `docker compose -f docker-compose.integration.yml --profile scylla config`; evidence `docs/test-evidence/scylladb.md`; implementation `336f4bd`. |
| T44 External product runner truthfulness | P0 | Done | Redshift and vendor Kafka scripts now distinguish a normal local SKIP from a strict requested run. The db-suite records missing external endpoints as skipped, while workflow-dispatch jobs fail closed when repository secrets are absent; regression output never echoes supplied DSNs. | `./scripts/integration-external-requirement-test.sh`; covered by `./scripts/verify.sh`; CI jobs set `DBTOOL_IT_REQUIRE_EXTERNAL=1`. |
| T45 Completeness ledger structural proof | P0 | Done | Replaced the old minimum-row validator with an exact 27-task contract: manifest/ledger IDs and statuses must agree in both directions, duplicate/extra/missing rows fail, COMPLETE evidence tables require cleanup/result proof, and full clones verify referenced commit ancestry. Mutation fixtures are part of `verify.sh`. | `./scripts/validate-db-completeness.sh`; `./scripts/validate-db-completeness-test.sh`; implementation `9ae1458`. |
| T46 Machine-readable CLI argument failures | P1 | Done | Added opt-in global `--json-errors`. Argument failures emit one stderr JSON envelope with stable `CLI_ARGUMENT_ERROR` and exit code 2; default Clap text plus help/version behavior remain compatible. | `cargo test -p dbtool-cli --test cli_format`; strict CLI test Clippy; implementation `97b2fa8`. |
| T47 Reproducible isolated product gates | P0 | Done | Pinned SQL Server to 2022 CU26, made Windows smoke return success after its expected rejected-write probe, and gave SQL Server, Cassandra/Scylla, Db2, and pure/native messaging runners the narrowest feature bundles so unrelated Db2 ODBC cannot break them before product access. Native messaging assertions now follow advertised capability operations instead of feature aliases. | `./scripts/validate-feature-matrix.sh`; SQL Server/Scylla run `29598991078`; current Windows run `29599157939`; Docker native messaging 11/11 + 4/4; implementations `7fcb072`, `bd41fc8`, `1fd88e6`, `9529b51`, `52f1f91`. |
| T48 Transitive dependency advisory remediation | P0 | Blocked upstream | Dependabot reports four `rustls-webpki` advisories in the shipped SQL Server/NATS dependency slots and one `lru` advisory in the unshipped TUI. Reachability is constrained but alerts remain open: current Tiberius has no fixed crates.io release, async-nats requires a reviewed MSRV/API jump, and stable Ratatui still pins lru 0.12. No incompatible forced lock update is allowed. | `docs/test-evidence/dependency-security-audit.md`; unblock with reviewed TLS/upstream upgrades plus product TLS and five-target regressions. |
| T49 Content-addressed container defaults | P0 | Done | Pinned every repository-default integration product, fixture base, TLS harness base, and published dbtool builder/runtime base as `tag@sha256`. A fail-closed validator plus eight mutation fixtures rejects floating, latest-without-digest, malformed, unresolved, and local-without-build defaults. Explicit environment/build-argument overrides remain developer escape hatches. | `./scripts/validate-container-image-pins.sh`; `./scripts/validate-container-image-pins-test.sh`; all 16 Compose configs; pinned dbtool Docker build/core smoke; implementation `a8f0ae7`. |
| T50 Patch release identity | P0 | Done | Preserved historical `v1.0.0`, bumped current source to `1.0.1`, made macOS ARM64 builds honor `Cargo.lock`, and reject an existing same-version tag unless it points at the exact current commit. Published `v1.0.1` from the all-green `6794f1b` source as an Apple Silicon GitHub prerelease. | `./scripts/validate-release-version.sh v1.0.1`; `./scripts/package-macos-arm64.sh v1.0.1`; CI `29905316297`; release `29906000948`; old `v1.0.0` tag remains immutable. |
| T51 npm package publication hardening | P1 | Done | Added dual-license payloads, explicit public npm registry metadata, canonical repository identity, concise wrapper failures, and a fail-closed seven-package matrix test covering platform mapping, tarball contents, offline host installation, binary override, and publish dry-runs. | `node ./scripts/package-npm-test.mjs`; evidence `docs/test-evidence/npm-publication.md`; implementation `8e631ac`. |
| T52 Public npm registry publication | P2 | External | Publishing is intentionally withheld until all six release binaries exist for one immutable version and the `@yovinchen` scope is authenticated/configured. Platform packages must publish before the wrapper; a macOS-only wrapper version cannot be backfilled later. | `npm whoami` currently returns `ENEEDAUTH`; follow `docs/npm-publication-tasks.zh-CN.md` after external account and artifact prerequisites are satisfied. |
| IF-T43–IF-T81 Interface completion campaign | P0 | Externally blocked | IF-T43–51 and IF-T53–81 are complete (38 tasks), including the selected macOS ARM64 release and Windows x64 runtime / ARM64 compile-link gates. IF-T52 alone remains externally blocked by unavailable product runtimes or DSNs. External prerequisites are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
