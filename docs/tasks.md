...
| IF-T43–IF-T75 Interface completion campaign | P0 | In progress | IF-T43 through IF-T67 plus IF-T69, IF-T71, and IF-T72 are complete. IF-T68 is implemented but still awaits the current SHA's Windows CI evidence. Remaining implementation is IF-T70 KV read envelopes, IF-T73 structured time-series budgets, IF-T74 catalog scalar bytes, and IF-T75 legacy API lifecycle. IF-T51 performs the final all-feature compile/test/package pass only after these interfaces close. External runtime prerequisites remain separately blocked and are never promoted to a pass. | `docs/interface-completion-tasks.zh-CN.md`; one independently verifiable feature per Lore commit. |
