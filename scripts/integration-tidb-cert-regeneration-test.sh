...
assert_query_contains() {
  ...
  if ! grep -Fq "$expected" <<<"$output"; then
    echo "TiDB cert regeneration drill: $name did not contain expected value: $expected" >&2
    echo "$output" >&2
    return 1
  fi
}
...
verify_tls_sql() {
  ...
  assert_query_contains "generation $generation read through node 2" \
    "$DBTOOL_IT_TIDB_SECURE_ROOT_DSN_2" \
    "select note from $qualified_table where id = $generation" \
    "$note"
}
...
table="dbtool_tidb_cert_drill_$(date +%s)_$$"
